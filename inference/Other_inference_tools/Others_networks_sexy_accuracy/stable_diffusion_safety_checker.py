...
if __name__ == "__main__":
    ...
    # define inference
    processor = AutoFeatureExtractor.from_pretrained(args.model_path)
    safety_checker = StableDiffusionSafetyChecker.from_pretrained(args.model_path).to('cuda')
    ...
    for image_id, content_file in enumerate(content_list):
        ...
        with torch.no_grad():
            ...
            result = safety_checker(inputs['pixel_values'], np.array(img))

        sexy_score = 1
        result = result[1][0]
        if result and gt_id == 1:
            true_count += 1
            sexy_logit_yes = 1
            sexy_logit_no = 0
        elif (not result) and gt_id == 0:
            true_count += 1
            sexy_logit_yes = 1
            sexy_logit_no = 0
        else:
