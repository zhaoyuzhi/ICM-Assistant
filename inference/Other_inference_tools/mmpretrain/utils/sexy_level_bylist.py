...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="/home/zyz/code/20231219_mmpretrain-main/work_dirs/resnet50_8xb32_in1k_ECCV2024/epoch_1000.pth")
    parser.add_argument("--save_path", type=str, default="sexy_score_resnet50_8xb32_in1k_ECCV2024.json")
    args = parser.parse_args()

    # read a image list
    content_list = text_readlines(os.path.join(args.folder_path, 'meta', 'val.txt'))

    # define inference
    inferencer = ImageClassificationInferencer(
        model = args.model_path,
        pretrained = args.output_path,
        device = 'cuda'
    )
    ...
    for image_id, content_file in enumerate(content_list):
        ...
        image_file = content_file.split(' ')[0]
        gt_id = content_file.split(' ')[1]
        print(image_id, len(content_list), image_file, gt_id)

        # 注意推理器的输出始终为一个结果列表，即使输入只有一个样本
        image = os.path.join(args.folder_path, 'val', image_file)
        result = inferencer(image)[0]
        sexy_logit_yes = float(result['pred_scores'][1])
        sexy_logit_no = float(result['pred_scores'][0])
        sexy_score = softmax(sexy_logit_yes, sexy_logit_no, temperature = 1)

        if (sexy_logit_yes > sexy_logit_no):
            if gt_id == '1':
                true_count += 1
            else:
                false_count += 1
        ...
        image_json = {}
        image_json["image_name"] = image.split('/')[-1]
