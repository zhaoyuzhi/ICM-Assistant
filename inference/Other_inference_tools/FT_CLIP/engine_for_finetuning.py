...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 200, header):
        ...
        target_array = target.cpu().detach().tolist()
        output_array = output.cpu().detach().tolist()
        for i, target_element in enumerate(target_array):
            if target_element == 0:
                logits_records["not sexy"].append(output_array[i])
            else:
                logits_records["sexy"].append(output_array[i])
...
@torch.no_grad()
def evaluate_per_image(args, data_loader, model, device):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for i, batch in enumerate(data_loader):
        ...
        # judge the results
        sexy_logit_yes = float(output_array[0][1])
        sexy_logit_no = float(output_array[0][0])
        sexy_score = softmax(sexy_logit_yes, sexy_logit_no, temperature=1)

        pred = True
        if sexy_logit_yes > sexy_logit_no and target[0] == 0:
            pred = False
        if sexy_logit_yes < sexy_logit_no and target[0] == 1:
            pred = False
        ...
        # save the results
        image_json = {}
        image_json["image_name"] = image_path
        ...
        print(image_path, sexy_logit_yes, sexy_logit_no, sexy_score, pred)
